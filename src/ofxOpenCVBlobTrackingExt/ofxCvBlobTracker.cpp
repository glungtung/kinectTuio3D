...
//	ofSetColor( 255,0,200,100 );
    ofxVec3f camToWorld(ofGetWidth()/640.0, ofGetHeight()/480.0, 1);
...
/*	ofSetColor(0xffffff);
    ...
    for( int i=0; i<blobs.size(); i++ ) {
        ...
        ofDrawBitmapString( docstring.str(),
                            blobs[i].centroid.x * camToWorld.x, blobs[i].centroid.y * camToWorld.y );
        ofCircle(blobs[i].centroid.x * camToWorld.x, blobs[i].centroid.y * camToWorld.y, 20);
    }
	glPopMatrix();
}
	...
	if( best_error_ndx != -1 ) {
		for( i=0; i<cursize; i++ ) {
			...
			if( blobs[i].id != -1 ) {
                ...
                blobs[i].centroid = oldblob->centroid + (blobs[i].centroid - oldblob->centroid) * lerp;
...
void ofxCvBlobTracker::findVelocity( int id, ofxVec3f* vec) {
	...
	vec->x  = 0.5 * blob.centroid.x - oldBlob.centroid.x;
	vec->y = 0.5 * blob.centroid.y - oldBlob.centroid.y;
...
void ofxCvBlobTracker::doBlobOn( const ofxCvTrackedBlob& b ) {
    if( listener != NULL ) {
        listener->blobOn( b.centroid.x, b.centroid.y, b.centroid.z, b.id, findOrder(b.id) );
    } else {
...
void ofxCvBlobTracker::doBlobMoved( const ofxCvTrackedBlob& b ) {
    if( listener != NULL ) {
        listener->blobMoved( b.centroid.x, b.centroid.y, b.centroid.z, b.id, findOrder(b.id) );
    } else {
...
void ofxCvBlobTracker::doBlobOff( const ofxCvTrackedBlob& b ) {
    if( listener != NULL ) {
        listener->blobOff( b.centroid.x, b.centroid.y, b.centroid.z, b.id, findOrder(b.id) );
    } else {
...
void ofxCvBlobTracker::setLerp( float v ) {
    lerp = v;
}
